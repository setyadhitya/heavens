...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  if (!verify_csrf($_POST['csrf_token'] ?? '')) {
    ...
    $UPLOAD_DIR_FS = $_SERVER['DOCUMENT_ROOT'] . '/heavens/uploads/tugas/kumpul_tugas/';
  ...
  <div class="container">
    ...
    <div class="row g-3">
              ...
              <form method="post" enctype="multipart/form-data" novalidate>
                ...
                <div class="mb-3">
                  ...
                  <select name="tugas_id" id="tugasSelect" class="form-select" required>
                    ...
                    <?php foreach ($tugas_terbuka as $t):
                      ...
                      if (!empty($t['kelas']))
                        $praktikum_label .= ' • Kelas ' . e($t['kelas']);
                      if (!empty($t['shift']))
                        $praktikum_label .= ' • Shift ' . e($t['shift']);
                      if (!empty($t['hari']))
                        $praktikum_label .= ' • ' . e($t['hari']);
                      $is_closed = (time() > strtotime($t['deadline']));
                      ?>
                      <option value="<?= (int) $t['tugas_id'] ?>" data-mk="<?= e($praktikum_label) ?>"
                        data-judul="<?= e($t['judul']) ?>" data-pertemuan="<?= (int) $t['pertemuan_ke'] ?>"
                        data-deskripsi="<?= e($t['deskripsi']) ?>" data-deadline="<?= e_dt($t['deadline']) ?>"
                        data-file="<?= e($t['file_tugas'] ?? '') ?>" data-closed="<?= $is_closed ? '1' : '0' ?>">
                ...
                <div id="tugasInfo" class="mb-3 d-none">
                  <div class="desc-box">
                    ...
                    <div class="mt-2">
                      <a id="btnLampiran" href="#" class="btn btn-sm btn-outline-primary" target="_blank"
                        rel="noopener">Download Lampiran Tugas</a>
                    </div>
                  </div>
                </div>
      ...
      <div class="col-lg-5">
                ...
                <table class="table table-sm align-middle">
                  ...
                  <tbody>
                    ...
                    foreach ($riwayat as $k):
                      ...
                      if (!empty($k['kelas']))
                        $praktikum_label .= ' • Kelas ' . e($k['kelas']);
                      if (!empty($k['shift']))
                        $praktikum_label .= ' • Shift ' . e($k['shift']);
                      if (!empty($k['hari']))
                        $praktikum_label .= ' • ' . e($k['hari']);
                      ?>
                      <tr>
                        <td><?= $no++ ?></td>
                        <td><?= e($praktikum_label) ?><br><span class="small-muted">Pert.
                            <?= (int) $k['pertemuan_ke'] ?></span></td>
                        <td><?= e($k['judul']) ?></td>
                        <td><?= e_dt($k['created_at']) ?></td>
                        <td>
                          <?php if (!empty($k['file_kumpul'])): ?>
                            <a class="btn btn-outline-primary btn-sm" href="<?= e($k['file_kumpul']) ?>" target="_blank"
        ...
        <div class="small text-muted mt-2">Catatan: Mengirim ulang tugas yang sama akan menimpa (overwrite) kiriman
          sebelumnya.</div>
      </div>
    </div>
  </div>
