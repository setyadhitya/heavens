...
function get_me(PDO $pdo, int $id) {
    $q = $pdo->prepare("SELECT id, username, nama, nim, nomorhp, password, role, status, created_at FROM tb_praktikan WHERE id = ? LIMIT 1");
    $q->execute([$id]);
    return $q->fetch();
}
$me = get_me($pdo, $praktikan_id);
if (!$me) {
        ...
        try {
            ...
            if ($field === 'username') {
                ...
                if ($username === '') {
                    ...
                    $st = $pdo->prepare("SELECT COUNT(*) FROM tb_praktikan WHERE username = ? AND id <> ?");
                    $st->execute([$username, $praktikan_id]);
                ...
                if (empty($errors)) {
                    $u = $pdo->prepare("UPDATE tb_praktikan SET username = ? WHERE id = ?");
                    $u->execute([$username, $praktikan_id]);
            ...
            if ($field === 'nama') {
                ...
                if (empty($errors)) {
                    $u = $pdo->prepare("UPDATE tb_praktikan SET nama = ? WHERE id = ?");
                    $u->execute([$nama, $praktikan_id]);
            ...
            if ($field === 'nim') {
                ...
                if ($nim === '') {
                    ...
                    $u = $pdo->prepare("UPDATE tb_praktikan SET nim = ? WHERE id = ?");
                    $u->execute([$nim, $praktikan_id]);
            ...
            if ($field === 'nomorhp') {
                ...
                if ($hp === '') {
                    ...
                    $u = $pdo->prepare("UPDATE tb_praktikan SET nomorhp = ? WHERE id = ?");
                    $u->execute([$hp, $praktikan_id]);
            ...
            if ($field === 'password') {
                ...
                if (empty($errors)) {
                    ...
                    $u = $pdo->prepare("UPDATE tb_praktikan SET password = ? WHERE id = ?");
                    $u->execute([$hash, $praktikan_id]);
...
<head>
<meta charset="utf-8">
<title>Profil Akun Praktikan</title>
...
<div class="container">
  <div class="header shadow-sm d-flex justify-content-between align-items-center">
    <div>
      <h4 class="mb-1"><i class="bi bi-person-badge"></i> Profil Akun Praktikan</h4>
  ...
  <div class="row justify-content-center">
    <div class="col-lg-7">
      ...
      <div class="text-center mt-3">
        <a class="small text-muted" href="/heavens/akun_mahasiswa/">&larr; Kembali ke Dashboard</a>
      </div>
    </div>
  </div>
</div>
