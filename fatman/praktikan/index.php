...
// Ambil data awal (render pertama), reload berikutnya via AJAX -> praktikan_action.php?action=list
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (formTambah) {
    formTambah.addEventListener('submit', async (e) => {
      e.preventDefault();
      try {
        const res = await fetch('praktikan_action.php', { method: 'POST', body: new FormData(e.target) });
  ...
  function attachTableListeners() {
    document.querySelectorAll('.btnEdit').forEach(btn => {
      btn.addEventListener('click', async () => {
        ...
        try {
          const res = await fetch('praktikan_action.php?action=get&id=' + id);
    ...
    document.querySelectorAll('.btnHapus').forEach(btn => {
      btn.addEventListener('click', async () => {
        if (!confirm('Yakin hapus praktikan ini? Tindakan ini tidak bisa dibatalkan.')) return;
        const form = new FormData();
        form.append('action', 'hapus');
        form.append('id', btn.dataset.id);
        form.append('csrf_token', '<?= e(csrf_token()); ?>');
        const res = await fetch('praktikan_action.php', { method: 'POST', body: form });
        const html = await res.text();
        document.getElementById('alertArea').innerHTML = html;
        await loadPraktikanTable();
      });
  ...
  async function loadPraktikanTable() {
    try {
      const res = await fetch('praktikan_action.php?action=list');
      const html = await res.text();
      document.getElementById('praktikanData').innerHTML = html;
