...
switch ($action) {
    ...
    case 'tambah':
        ...
        // Unik username
        $stmt = $mysqli->prepare("SELECT COUNT(*) AS jml FROM tb_praktikan WHERE username = ?");
        ...
        // Unik NIM
        $stmt = $mysqli->prepare("SELECT COUNT(*) AS jml FROM tb_praktikan WHERE nim = ?");
        ...
        if ($ins->execute()) {
            echo '<div class="alert alert-success">✅ Praktikan berhasil ditambahkan (status: aktif).</div>';
        } else {
    ...
    case 'get':
        $id = (int)($_GET['id'] ?? 0);
        $res = $mysqli->prepare("SELECT id, username, nama, nim, nomorhp, status FROM tb_praktikan WHERE id = ?");
        $res->bind_param("i", $id);
        $res->execute();
    ...
    case 'edit':
        ...
        $stmt = $mysqli->prepare("SELECT COUNT(*) AS jml FROM tb_praktikan WHERE username = ? AND id <> ?");
        ...
        $stmt = $mysqli->prepare("SELECT COUNT(*) AS jml FROM tb_praktikan WHERE nim = ? AND id <> ?");
        ...
        if ($upd->execute()) {
            echo '<div class="alert alert-warning">✅ Data praktikan berhasil diperbarui.</div>';
        } else {
    ...
    case 'hapus':
        ...
        $del = $mysqli->prepare("DELETE FROM tb_praktikan WHERE id = ?");
        $del->bind_param("i", $id);
        if ($del->execute()) {
            echo '<div class="alert alert-danger">Data praktikan telah dihapus.</div>';
        } else {
    ...
    case 'list':
        $result = $mysqli->query("SELECT * FROM tb_praktikan ORDER BY id DESC");
