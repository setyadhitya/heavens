...
$assistenRes = $mysqli->query("
  SELECT id, nim, nama
  FROM tb_assisten
  WHERE status = 'aktif'
  ORDER BY id DESC
");
$assistenList = $assistenRes ? $assistenRes->fetch_all(MYSQLI_ASSOC) : [];
?>
<!doctype html>
<html lang="id">
<head>
...
<!-- 🔹 Modal Tambah (Pilih 1 Praktikum, bisa tambah banyak Assisten) -->
    ...
    <div class="modal-content">
      <form id="formTambah" method="post" class="p-3">
        ...
        <div class="mb-3">
          ...
          <div class="small-muted">Pilih satu praktikum, lalu tambahkan beberapa assisten sekaligus.</div>
        </div>

        <!-- Daftar Assisten (dinamis) -->
        ...
        <div id="assistenRows" class="vstack gap-2">
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function attachRemoveRowHandlers() {
    rowsContainer.querySelectorAll('.btnRemoveRow').forEach(btn => {
      btn.onclick = () => {
        const row = btn.closest('.assisten-row');
        if (row) {
          if (rowsContainer.querySelectorAll('.assisten-row').length <= 1) {
